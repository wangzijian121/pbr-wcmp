...
public class LoginController extends BaseController {
    ...
    @PostMapping(value = "/wcmp/login")
    @ResponseStatus(HttpStatus.OK)
    public Result wcmpLogin(@RequestParam String username,
    ...
    @PostMapping(value = "/wcmp/wcmpLogout")
    @ResponseStatus(HttpStatus.OK)
    public Result wcmpLogout(@ApiIgnore @RequestAttribute(value = "session.userId") int userId, HttpServletRequest request) {
        // user ip check
        String ip = getClientIpAddress(request);
        if (StringUtils.isEmpty(ip)) {
            return error(10125, "Cant find ip！");
